...
export function ReactionChart({
  ...
  const positivePercent = allCount === 0 ? 0 : (positiveCount / allCount) * 100;
  ...
  const percent =
    allCount === 0 ? 0.001 : Math.min(0.999, Math.max(0.001, positiveCount / allCount));
            ...
            {isInView && (
              <g transform={!isPositive ? 'translate(208, 0) scale(-1,1)' : undefined}>
                <motion.path
                  ...
                  pathLength={1}
                  initial={{
                    strokeDasharray: !disabled ? '0.001 1' : `${percent} 1`,
                    strokeDashoffset: 0,
                  }}
                  animate={{
