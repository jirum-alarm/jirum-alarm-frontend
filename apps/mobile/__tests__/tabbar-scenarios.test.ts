...
describe('★왕복 시나리오 — 이전 상태가 남지 않는가', () => {
  ...
  it('★탭 왕복 — 홈 → 상세 → 홈 → 상세 → 홈 (10회)', () => {
    ...
    const states = walk(seq);
    // 마지막은 반드시 보임
    expect(states.at(-1)).toBe(true);
    // 홈 자리는 전부 true, 상세 자리는 전부 false
    expect(states.filter((_, i) => i % 2 === 0).every(Boolean)).toBe(true);
    expect(states.filter((_, i) => i % 2 === 1).some(Boolean)).toBe(false);
  });

  it('상세 → 댓글 → 상세 (숨김끼리 이동해도 중간에 안 켜진다)', () => {
  ...
  it('★다른 탭에 상세가 열려 있어도 홈은 영향 없다', () => {
    // 탭별 스택이 독립이므로 "지금 보는 탭의 최상단"만이 답이다.
    const discoverTop = ROUTES.DETAIL; // 발견 탭엔 상세가 열려 있음
    const homeTop = ROUTES.ROOT; // 홈 탭은 루트

    // 홈을 보고 있을 때의 답은 홈 스택 기준이어야 한다
